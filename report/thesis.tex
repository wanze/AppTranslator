...
\item \textbf{Recombination} Matching hypothesis both having the same English words in the output can be reached by different paths. In the heuristic search they can be recombined and the worse hypothesis is dropped (see Figure~\ref{fig:related_work_hypothesis_recombination}).
...
\begin{figure}[H]
    ...
    \caption[Decoding process: Apply Recombination of hypotheses \cite{smt_book_koehn}]{Example of possible Recombination: The output ``it is'' can be reached with two different paths. The score of the translation option using a single phrase is worse, so this hypothesis is dropped  \cite{smt_book_koehn}.}
...
\begin{enumerate}
\item Building a language model from the monolingual data of the target language. Moses includes the \emph{KenLM toolkit}\footnote{\url{https://kheafield.com/code/kenlm/} (accessed 13 May 2016)} for this task.
...
The decoder uses \emph{Recombination} and \emph{Beam Search} algorithms (Section~\ref{sec:phrase_based_decoding}) to reduce the size of the search space. 
...
\rp{Time depends on the performance of a particular machine, so you either need to specify the characteristics of a machine, or remove this.}
Training one model (e.g. from English to French) took about two days with the parameters indicated above. At each checkpoint, when Tensorflow saves the current model to disk, some statistics are printed:
...
The Baseline System needed a special setup. First, the development set is ignored since there is no tuning involved in this model. \rp{And? what do you do about this?} Second, the training data was indexed separately inside Solr for each evaluation run. This was achieved by creating new cores in Solr according to the following schema:

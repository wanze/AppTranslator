...
\item Android application keeps sentences in key-value format in a single XML file per language. These files can be out of sync, the developer could either forgot to add or to remove a translation.
\end{enumerate}
...
Overall, a total of 114'947 parallel sentences were collected for the English-French language pair and 114'773 for the English-German one. The monolingual corpus holds 187'642 English sentences, 126'883 French sentences and 126'975 German sentences. Again, the counts are measured after pre-processing the extracted data (Section~\ref{sec:preprocess_translations}).
...
An Android app represents a single archive file (suffix \texttt{.apk}). The translations are stored in XML files and these files are packed inside the \texttt{apk} binary. A specific software such as \emph{Apktool}\footnote{\url{http://ibotpeaches.github.io/Apktool/} (accessed 13 May 2016)} can be used to extract the XML files from an APK file.
...
Solr is also used as \emph{Baseline Translation System}, where strings can be translated with a simple algorithm directly from the collected data in Solr (Chapter~\ref{cha:translation_process_baseline}).
...
The \emph{Baseline System} makes direct use of the data stored in Solr and produces translations with a simple algorithm. Recall how the data is organized in Solr (Section \ref{sec:apache_solr}): Each translation is stored in a document and each document belongs to a one core. To look up a translation, the following procedure is used:
...
\end{enumerate}
Figure \ref{baseline_system_table_available_translations} shows some translations for the English word ``Settings''. The best translation according to the highest count among all apps is ``param\`{e}tres''. Note that the parameter ``number of rows'' influences performance and the translation result: Setting low values increases performance because less queries are needed when searching for available translations in the target language. However, the best translation might not be the correct one, as there are not enough translations available when counting unique values.
...
\begin{enumerate}
\item Building a language model from the monolingual data of the target language. Moses includes the \emph{KenLM toolkit}\footnote{\url{https://kheafield.com/code/kenlm/} (accessed 13 May 2016)} for this task. We built 3-gram language models as used in~\citep{smt_phrase_based}.
...
\item Scoring phrases by computing the phrase translation probability \(\phi(\bar{f}|\bar{e})\) and building a phrase table. Below is an example how the content of the phrase table is organized in Moses for the English-French language pair:
...
In the next section, we describe the model tuning process for finding optimal weights.
...
\begin{itemize}
\item \(\phi(f|e)\) The \textbf{phrase translation table} is responsible that the input and output phrases are good translations of each other by looking at the probability scores~\citep{pdf_moses_manual}.
...
The decoder uses \emph{Recombination} and \emph{Stack Pruning} algorithms (Section~\ref{sec:phrase_based_decoding}) to reduce the size of the search space. 
...
\begin{itemize}
\item We skip the part where Tensorflow downloads and prepares the sample data since we train models on our corpus (Chapter~\ref{cha:data_extraction_prep_storage});
...
We used the default buckets from the tutorial \citep{website_tensorflow_tutorial}: \texttt{[(5, 10), (10, 15), (20, 25), (40, 50)]}
\\
...
As in the previous chapters, we translate the example phrase ``Open the settings to change your usernam'' to French.
...
\rp{Maybe add something about the performance of neural networks/tensorflow}

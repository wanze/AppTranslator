...
\item Recursively start to translate substrings of \(S\). Let \(l\) be the number of words in \(S\) at any recursion level. We check for translations of any possible substrings \(\bar{S}_i\) of length \(l - 1\). If we find translations, we insert them into \(T\) at the position where the substring started. Note that if we find translations for multiple substrings, we translate the ones having the highest total count.
\item The above step is repeated until there are left only single words to translate (or in other words, substrings of length 1). If we cannot find a translation at this point, the source word \(s_i\) is inserted in \(T\).
\end{enumerate}

\subsection{Example}

Let us translate the source string ``There is no internet connection available, check your Wi-Fi settings'' from English to French. 


%There is no internet connection available, check your settings.%

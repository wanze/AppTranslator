...
To reduce the scope of different mobile operating systems, focus shall be put on automatic translations of Android apps. Hereinafter, the term \emph{app} in this thesis refers to an Android application.
...
This chapter first describes the importance of app localization. Next, translation services that are already available for this task are analysed. Finally, SMT is introduced.
...
Crowdsourcing platforms allow people to perform various tasks, including text translation. This approach can be used in combination with machine translation: Initial translations are produced with machine translation, which are then improved or corrected by human translators. Facebook used crowdsourcing to translate their website into French by allegedly over 4'000 native French speakers\footnote{\url{https://en.wikipedia.org/wiki/Crowdsourcing_as_Human-Machine_Translation} (accessed 12 May 2016)}. A few examples of crowdsourcing translation platforms are \emph{Translation Cloud}\footnote{\url{http://www.translationcloud.net/} (accessed 13 May 2016)} and \emph{Gengo}\footnote{\url{http://gengo.com/} (accessed 13 May 2016)}. Using these platforms, people who are fluent in at least two languages can earn money by translating or correcting texts. To avoid cheaters, an initial language proficiency test must be passed.
...
Before phrase-based models were introduced, machine translation was based on the translation of single words. However, using words as the smallest unit for translation has limitations. Often one word in the source language translates to multiple words in the target language, or vice versa. Phrase-based translation overcomes this problem by translating whole sequences of words of different lengths. These sequences are called phrases. The sentence to translate is first segmented into multiple phrases. Each phrase is then translated separately into a phrase in the target language. Phrases are looked up in a \emph{phrase translation table}. %containing all candidate phrases for a translation%.
Finally, phrases may be reordered. Reordering is necessary due to grammatical differences in the source and target language where the position of the subject, verb or object in a sentence differs  \citep{smt_book_koehn}.

The following sections describe the phrase-based model in more detail.
...
\[P(e|f)\]
To find the best English translation \(e_{best}\), the following equation based on the Bayes rule is used:
...
where \(\phi\) is called the \emph{phrase translation probability} and \(d\) is the \emph{reordering probability}. %\rp{maybe you can introduce why do we need re-ordering.}
...
%\rp{I think the translation table concept was not introduced before, then you need to introduce it briefly.}

Given a phrase to translate from the input sentence, the phrase translation table contains all possible phrase translations, together with a probability score (see Table \ref{fig:phrase_table_example}). The higher the score, the more often the phrase was used to translate the input phrase in the parallel corpus. However, the phrase having the highest probability score must not be the best translation. The reordering model and language model also contribute to the final score. Building a phrase translation table involves three stages:
\begin{enumerate}
...
\item the / les
\end{itemize}
The last step is to compute the phrase translation probabilities based on relative frequency:
...
\end{tabular}
\caption[Example of a phrase translation table]{Example of a phrase translation table containing the computed probabilities for translating the English word ``Settings'' to French.}
...
\item \textbf{Language model} A \emph{n}-gram model needs to keep track of the last \(n - 1\) words that were added to the derived sentence. The score is computed for every added word \(w_i\): \(p_{LM}(w_i|w_i - (n-1), ..., w_{i-1})\)

%\item \textbf{Language model} For a \emph{n}-gram model, we need to keep track of the last \(n - 1\) words. The score is computed for every added \rp{added where?} word \(w_i\): \(p_{LM}(w_i|w_i - (n-1), ..., w_{i-1})\)
\end{itemize}
...
\end{figure}
This process creates an exponential number of hypothesis. To reduce the search space, the following methods are typically applied:

\begin{itemize}
%\item \textbf{Recombination} Two hypothesis paths lead to two matching hypotheses, so that both hypotheses have the same English words in the output. The worse hypothesis is dropped \rp{I'm not sure I understand the method here, could you expand?}.
\item \textbf{Recombination} Hypothesis both having the same English words in the output can be reached by different paths. In the heuristic search they can be recombined and the worse hypothesis is dropped \cite{smt_book_koehn} (see Figure~\ref{fig:related_work_hypothesis_recombination}).
\item \textbf{Stack Pruning} Bad hypotheses are removed early. Hypothesis that have translated the same number of input words are put into a stack, so that they are comparable. The number of hypotheses in a stack is limited, for example by keeping only \emph{k} hypotheses according to a score. This score is typically a combination of the partial probability score and a future cost estimation for translating the remaining sentence \cite{smt_book_koehn}.
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/related_work_recombination.png}
    \caption[Decoding process: Apply Recombination of hypotheses \cite{smt_book_koehn}]{Example of Recombination: The output ``it is'' can be reached with two different paths, one path using a single phrase and the other path using two separate phrases. The score of the translation option using a single phrase is worse, so the hypothesis is dropped  \cite{smt_book_koehn}.}
    \label{fig:related_work_hypothesis_recombination}
\end{figure}
...
The next section briefly describes how RNNs work in general while the following sections explain the encoder-decoder architecture in more detail.
...
A RNN is a class of artificial neural networks where connections between units form a directed cycle. A RNN consists of a hidden state \(h\) and an optional output \(y\), which operates on a variable-length sequence \( x = (x_1, ..., x_n)\). At each time step \(t\), the hidden state \(h_{t}\) is updated by the following function:
...
The encoder is a RNN that reads each word of the source sentence sequentially in a form of a \emph{one-hot vector}. This binary vector has the size of the vocabulary and zeros everywhere except a \(1\) at the position corresponding to the index of the word in the source vocabulary.

The internal state \(h_t\) changes each time a new word is read:

\[  h_{t} = f(h_{t-1}, x_t) \]
After processing each word, the hidden state of the RNN is a summary \(c\) of the source sentence (see Figure \ref{fig:related_work_encoder}). 
...
\end{figure}
It is very interesting how the summary vector \(c\) looks like. In~\cite{smt_deep_learning}, the authors project multiple summary vectors into the two-dimensional space (see Figure \ref{fig:related_work_summary_vector}). The plot shows that similar sentences are close together in the summary vector space.
...
The decoder is another RNN which is trained to generate the output sentence by predicting the next word \(y_t\) given the hidden state \(h_t\). Both \(y_t\) and \(h_t\) are conditioned on \(y_{t-1}\) and the summary vector \(c\) given from the encoder. The hidden state of the decoder is computed by
...
The proposed encoder-decoder system is trained by maximizing the \emph{log-likelihood}\footnote{\url{https://en.wikipedia.org/wiki/Likelihood_function} (accessed 13 May 2016)} of the training corpus. Training requires a parallel corpus \(C\) of source and target sentences \((X^n, Y^n)\). The sentences are represented as sequence of one-hot vectors. The conditional log-probability of \(Y^n\) given \(X^n\) can be computed by \( log P(Y^n|X^n, \theta) \). The log-likelihood of the training corpus is written as:
...
This chapter presents statistics regarding the collected apps and extracted translation data. Section~\ref{sec:data_analysis_languages} shows the most popular languages that were used to translate the collected apps. As German and French are both in the top four, the focus of this thesis is put on translations from English to French and German. Beside having a good amount of data available to train a SMT system later, French and German are natively spoken in Switzerland, which makes understanding the produced results easier.

\section{Apps}

For the purposes of this work, a total of 698 free Android apps were collected from various categories, ensuring a good mix of different sentences. Figure~\ref{fig:data_analysis_top_apps_en} shows the top ten apps according to the number of available English translations. Note that the counts indicate the number of effective sentences after eliminating non-relevant ones through the pre-processing step (see Chapter~\ref{cha:data_extraction_prep_storage}). Most of the apps have similar counts for all three languages. Surprisingly, some apps provide more sentences for German and French than English. Different counts can happen because of:

\begin{enumerate}
\item Not all strings need to be translated, some English words remain the same in the target language e.g. ``E-Mail''.
\item Android applications store sentences in a key-value based format in a single XML file per language. These files can be out of sync, if the developer forgets to add or remove a translation.
\end{enumerate}
...
\section{Languages}
\label{sec:data_analysis_languages}

A total of 423 multilingual apps were collected, offering its content in at least two languages. Figure \ref{fig:data_analysis_n_langs} shows the number of apps for the ten most used languages. The top apps are translated in many more languages. Overall, 117 different languages were available.

%A total of 423 multilingual apps were collected, offering its content in at least two languages. Figure \ref{fig:data_analysis_n_langs} shows the number of apps for the ten most used languages. French and German, the two languages that were the main focus of this thesis 
%\rp{I think you never mentioned why French or German, because the thesis in not actually language specific, and you need to also state it somewhere, but just write that we used these languages as ones of the most popular for translations, so they are the main focus as a consequence of being in top four, not vice versa.}, are both in the top four. The top apps are translated in many more languages. Overall, 117 different languages were available.
...
Overall, a total of 114'947 parallel sentences were collected for the English-French language pair and 114'773 for the English-German one. The monolingual corpus holds 187'642 English sentences, 126'883 French sentences and 126'975 German sentences. Again, the counts are measured after pre-processing the extracted data (see Section~\ref{sec:preprocess_translations}).
...
The lengths were measured in the monolingual corpus after tokenization, and punctuation is also included in the counts.
...
This chapter describes the process to extract and store translation data from existing apps. The resulting parallel corpus holding the English-French and English-German sentences is required to train SMT models.

\section{Extract Translations}

An Android app is represented by a single archive file (suffix \texttt{.apk}). The translations are stored in XML files and these files are packed inside the \texttt{apk} binary. A specific software such as \emph{Apktool}\footnote{\url{http://ibotpeaches.github.io/Apktool/} (accessed 13 May 2016)} can be used to extract the XML files from an APK file.
...
\end{figure}
Translations are stored in a key-value format inside the \texttt{strings.xml} files. Each string has a unique key which holds the translation value. Below is an example of an English XML file, followed by its German counterpart:
...
\section{Translation Pre-processing}
\label{sec:preprocess_translations}
%\rp{maybe tell again why do we need pre-processing here in detail.}

Pre-processing the translations is needed because the strings in the XML files are not only stored as plain text. A lot of apps use HTML to format text. For the purpose of translating, any HTML tags must be removed, otherwise they become part of words. The task of translation pre-processing involves two steps. First of all, the translations are sanitized from unwanted information. Second, the sanitized strings are tokenized so that they can be used to train a SMT system.

\subsection{Sanitizing}

The following steps are used to remove unwanted information:

\begin{itemize}
\item Trimming newlines (\texttt{\textbackslash n}), tabs (\textbackslash t) and carriage returns (\textbackslash r);
\item Stripping HTML tags;
\item Removing URLs (starting with http:// or www.);
\item Removing string placeholders, such as \texttt{\%s} and \texttt{\%1\$s};
\item Ignoring words that are less than 3 characters;
...
%\rp{write why do we need tokenization and truecasing, it is important to always tell why do this and that.}

Tokenization is required to separate words from punctuation. Each token then is separated by a space character, e.g, ``\texttt{Hi, my name is John.}'' becomes ``\texttt{Hi , my name is John .}'' 
\\
In machine translation, tokenization prevents a system to learn different translations for the same words. For the example above, without tokenization, ``Hi'' and ``Hi,'' would be considered as two different words.   

After tokenization, \emph{truecasing}\footnote{\url{https://en.wikipedia.org/wiki/Truecasing} (accessed 13 May 2016)} was applied to the text corpus. In contrast to lowercasing, truecasing determines the proper capitalization of words. This is done by first analyzing how words are mostly capitalized in the corpus. The proper case is then applied to each word. It is especially useful for words starting a sentence, that are written uppercase in many languages. For example the word ``This'' starting a sentence will become ``this'', because it is usually written lowercase. Again, in the context of machine translation, normalizing the words to have a common capitalization prevents a system to learn different representations of the same word.
...
After pre-processing the translations, they are written to text files, one sentence per line. The corpus is split into parallel and monolingual data. A parallel corpus (often also called bitext) represents sentence aligned text-files of one language pair. All sentences must be aligned so that line \(x\) of the first language corresponds to line \(x\) of the second language. In order to get a random mix of translations from different apps, the sentences are shuffled before they are written to the files. The parallel corpus can be used by any SMT system to train the models. If the model requires a separate development set of parallel sentences, the existing corpus can be split at any percentage, e.g. use 80\% as training data and the remaining 20\% as development set.
In addition, data of each language is also stored in a monolingual corpus. All sentences of one language are written into a single text file. This data is then used to build language models, which are an important part of a phrase-based translation model (see Chapter~\ref{sec:phrase_based_mt}).
...
The previous section described how to store the pre-processed translation data in text files which can be processed by an SMT system. In order to perform data analysis, this format is not practical, because the sentences in the corpora are no longer linked to the app they are coming from. Ideally, all extracted translations are additionally stored in a database, offering tools to analyse indexed data. \emph{Apache Solr}\footnote{\url{http://lucene.apache.org/solr/} (accessed 13 May 2016)} was chosen for this purpose. Solr is an open source text search platform. Built in tools allow to perform data analysis, for example to get the top sentences grouped by language.


%\rp{describe first what we are trying to achieve here, like, we need to store data in a scalable way, with a possibility to perform this and that, and access this and that. We chose Apache Solr for such purposes.}
...
%Built in tools allow to perform data analysis, for example to get the top sentences grouped by language.

Solr is also used as \emph{Baseline Translation System}, where strings can be translated with a simple algorithm directly from the collected data in Solr (see Chapter~\ref{cha:translation_process_baseline}).

\subsubsection{Setup and Schema}

Data in Solr is stored in so-called \emph{cores}, where each core manages a separate index, schema and configuration\footnote{\url{https://cwiki.apache.org/confluence/display/solr/Solr+Cores+and+solr.xml} (accessed 13 May 2016)}. Our setup holds one core per language and one document per translation string. The schema is identical for each core (language) and consists of the following fields:

\begin{itemize}
\item \textbf{id} A unique ID used by Solr to identify a document.
\item \textbf{app\_id} A unique app ID that allows to map a given translation to the app it is coming from.
\item \textbf{key} A translation key from the XML file.
\item \textbf{value} A translation value (word or sentence) corresponding to the above key.
\item \textbf{value\_lc} Again the translation value, additionally indexed with a special start- and end- character. This allows to search for exact values of any translation string, as described in the next section.
\end{itemize}
...
Here is an example query for the string ``Log Out'' from the English core. The maximum amount of results is set to 500 rows and the return type is JSON:
\\
\\
\url{http://SOLR_URL/solr/en/select?q=value:"Log\%20Out"&rows=500&wt=json&indent=true}
\\
\\
This query finds a total of 458 results (see Listing \ref{lst:solr_json_response}). As can be seen, not only exact matches are returned, the term ``Log Out'' can occur as substring in a longer sentence. By default, Solr does not offer the possibility to return only exact matches. However, this behavior can be simulated with the help of the \texttt{value\_lc} field, which indexed the translation value together with a special start- and end character (\texttt{SOLR\_S} and \texttt{SOLR\_E}):
\\
\\
\url{http://SOLR_URL/solr/en/select?q=value_lc:"SOLR_S\%20Log Out\%20SOLR_E"&rows=500&wt=json&indent=true}
\\
\\
This query returns only 396 results, representing exact matches. Note that the casing of the search terms does not matter and whitespaces and punctuation signs are ignored. Executing the above query with the term ``log Out .'' produces the same results. This procedure is especially useful when looking up translations, since proper case or punctuation needs no consideration.
...
The \emph{Baseline System} makes direct use of the data stored in Solr and produces translations with a simple algorithm. Recall how the data is organized in Solr (see Section \ref{sec:apache_solr}): Each translation is stored in a document and each document belongs to a one core. To look up a translation, the following procedure is used:

\begin{enumerate}
\item Look up the string to translate in the core of the source language.
\item Parse the resulting documents and extract the \texttt{app\_id} and \texttt{key} fields. The number of documents returned from Solr can be set by a parameter ``number of rows'', the default chosen for the Baseline System is 100 documents.
...
\end{enumerate}
Figure \ref{baseline_system_table_available_translations} shows some translations for the English word ``Settings''. The best translation according to the highest count among all apps is ``param\`{e}tres''. Note that the parameter ``number of rows'' influences performance and the translation result: Setting low values increases performance because less queries are needed when searching for available translations in the target language (Step 3). However, the best translation might not be the correct one, as there are not enough translations available when counting unique values (Step 4).
...
Now, we search for translations for all substrings in both variations \(V_1\) and \(V_2\). If no translations are found, the substring (window) size is reduced by one for the second iteration (\(i = 2\)):
...
The process of reducing the substring length is repeated until we can translate a substring of at least one variation \(V_j\).

\item Let us consider what happens if there are translations found for at least one Variation \(V_j\). For each variation \(V_j\), we count the number of available translations for their substrings \(\bar{S}_i\). We continue with the variation \(\tilde{V}\) having the highest total count of translations among their substrings:
...
Otherwise, the output string \(T\) is built incrementally: If no translation is found for substring \(\bar{S}_i\), we set \(S = \bar{S}_i\) and recursively call the algorithm again. If single words are left to translate and still no translations exist, the source word is inserted into \(T\) by setting \(t_i = s_i\).

\end{enumerate}
This simple algorithm works well for translating short words or sentences where a direct translation exists in the target language.
However, if substring translation is necessary (Step 2 of algorithm), the following problems can occur:

\begin{itemize}
\item Sentences or words for translated substrings are inserted at the same position in \(T\) where they occurred in the source string. This will not produce good results since words often need to be reordered depending on the target language.
\item Poor app translation quality directly impacts the translation results. For example, due to an error in the XML translation files, the English word ``the'' could be mapped to a sentence with many words. If the algorithm looks up ``the'', it considers the mapped sentence as valid translation.
\end{itemize} 


\subsection{Example}

Let us translate a string ``Open the settings to change your username'' from English to French.
...
\end{enumerate}
At the last step, there are translations available for both substrings of the variation \(V_1\) and the algorithm returns the following string: ``ouvrir les param\`{e}tres vers modifier votre nom d'utilisateur''


\section{Moses}

\emph{Moses}\footnote{\url{http://www.statmt.org/moses/} (accessed 13 May 2016)} \cite{moses} is a open source SMT toolkit. The training process closely follows the phrase-based model described in Section~\ref{sec:phrase_based_mt}. The source code is available on \emph{GitHub}\footnote{\url{https://github.com/moses-smt/mosesdecoder} (accessed 13 May 2016)}. The project is very active, and there are frequent updates in the main branch by over 70 contributors.
...
\begin{enumerate}
\item Building a language model from the monolingual data of the target language. Moses includes the \emph{KenLM toolkit}\footnote{\url{https://kheafield.com/code/kenlm/} (accessed 13 May 2016)} for this task. We built trigram language models as used in~\citep{smt_phrase_based}.
\item Aligning words of the parallel sentences. We used an external library \emph{MGIZA}\footnote{\url{https://github.com/moses-smt/mgiza.git} (accessed 13 May 2016)}, an extension of the popular \emph{GIZA++} word alignment toolkit \cite{giza_pp}. MGIZA is multi-threaded and therefore runs faster on a multi-core machine.
\item Extract phrases: All phrases are put into a single file. The following listing shows some sample content of this file. Each line of this file contains: an English phrase, a French phrase and the alignment points of matching words (English-French).
...
\item Scoring phrases by computing the phrase translation probability \(\phi(\bar{f}|\bar{e})\) and building a phrase table. Below is an example how the content of the phrase table is organized in Moses for the English-French language pair. Note that the scores are shortened to two decimals in this example.
...
! your version of ||| ! votre version de ||| 1 0.04 1 0.24 
! your version ||| ! votre version ||| 1 0.63 1 0.50 
! your ||| ! ton ||| 1 0.59 0.04 0.004 
! your ||| ! votre ||| 1 0.79 0.73 0.54
! your ||| ! your ||| 1 0.89 0.13 0.01
! your ||| . vos ||| 0.04 0.004 0.04 0.01
\end{lstlisting}
...
The \emph{lexical weighting} is an additional score to check the reliability of the phrase translation probability. Rare phrase pairs may cause problems if they are collected from noisy data. If both phrases \(\bar{f}\) and \(\bar{e}\) occur only once in the training data, then \(\phi(\bar{f}|\bar{e}) = \phi(\bar{e}|\bar{f}) = 1\). This often overestimates how reliable rare phrase pairs are. Lexical weighting decomposes phrase pairs into its word translations, so that we can check how well they match~\citep{smt_book_koehn}.
...
The next section describes the model tuning process.
...
\begin{itemize}
\item \(\phi(f|e)\) The \textbf{phrase translation table} is responsible that the input and output phrases are good translations of each other by checking the probability scores~\citep{pdf_moses_manual}.
\item \(LM(e)\) The \textbf{language model} makes sure that the produced output sentence of the target language is fluently~\citep{pdf_moses_manual}.
\item \(D(e,f)\) The \textbf{distortion model} allows for reordering of the input sentence, but at a cost. The more reordering, the more expensive is the translation~\citep{pdf_moses_manual}.
\item \(W(e)\) The \textbf{word penalty} ensures that the translations do not get too long or too short. Negative values favor longer output while positive values favor shorter output~\citep{pdf_moses_manual}.
\end{itemize}
There is a weight assigned to each of these components that influences their importance. These weights can be passed to the decoder when translating sentences. The optimal weights depend on the languages and training corpus. However, Moses offers a possibility to optimize the weights automatically by maximizing the BLEU score \cite{bleu_score} on a small, separate set of parallel sentences (tuning set).

We used 80\% of the available parallel data for training and the remaining 20\% for tuning. Tuning process takes significantly more time than the training. Moses executes different tuning runs and measures the BLEU score achieved on the tuning set. The optimal weights were usually determined after 6-11 runs. %, which took about 3-6 hours.

\subsection{Decoding}

The decoder of Moses is controlled by the \texttt{moses.ini} file. This file stores the path to the phrase translation table and the language model. Additionally it also holds the tuned weights for the translation model. Many more configuration options\footnote{\url{http://www.statmt.org/moses/?n=Moses.DecoderParameters} (accessed 13. May 2016)} can be set in this file or passed as parameters when executing the decoder. Here is a summary from \cite{pdf_moses_manual} how the decoder works:
...
\end{itemize}
\item The output sentence is generated left to right in a form of several hypotheses. Each hypothesis is represented by:
\begin{itemize}
\item A link to the best previous state, so that the decoder later can backtrack through the graph to find the best translation.
...
The decoder uses \emph{Recombination} and \emph{Stack Pruning} algorithms (see Section~\ref{sec:phrase_based_decoding}) to reduce the size of the search space. 
...
\end{enumerate}
\textbf{Example} We translate the same example phrase as in the previous chapter ``Open the settings to change your username'' to French. The command to invoke the decoder is the following:
...
\end{lstlisting}
We used stack pruning with a stack size of 100 and the optimal model weights determined after tuning.

The decoder produces the following output phrase: ``ouvrir les param\`{e}tres pour modifier votre nom d' utilisateur''.
Here are some statistics on how the decoder derived the target sentence.
...
As we can see, 197 phrase translation table entries (translation options) are collected by Moses. During decoding, a large amount of hypotheses are created. Many are discarded early because they were considered bad, some others were pruned or recombined at a later stage.
...
Tensorflow includes a package to learn sequence to sequence models with neural networks. The implementation follows the encoder-decoder RNN model described in Section~\ref{sec:related_work_deep_learning}.
...
However, the training script was modified in a number of ways to suit the needs of this work:
\begin{itemize}
\item We skip the part where Tensorflow downloads and prepares the sample data since we train models on our corpus (see Chapter~\ref{cha:data_extraction_prep_storage});
\item We replace the included tokenizer by registering a function that just splits a string at spaces, because our corpus is already tokenized;
\item We added additional parameters for the training process to make the training script more generic. New parameters include the source and target language and the path to the parallel corpus.
\end{itemize}
Training requires the following model parameters (values in square brackets represent the chosen default values):. 
...
At each checkpoint, when Tensorflow saves the current model to disk, some statistics are printed:
...
%\rp{You need to tell what (5,10) is, what is 5 and what is 10, why the target length is bigger than the source, the phrases are of the same length, it's not straightforward. And why there is GO in a french sentence?}

The statistics show the current \emph{perplexity} of different \emph{buckets} at timestep \(t\). Tensorflow uses bucketing in combination with padding to efficiently handle sentences of different lengths. Each sentence pair from the training data is put into a bucket according to the length of the source and target sentence. The sentences are then padded to fit the dimension of the bucket. %The bucket \((x, y)\) contains sentences of length \(x\) in the source language and sentences of length \(y\) in the target language.
We used the default buckets from the tutorial~\citep{website_tensorflow_tutorial}:
\\
\texttt{[(5, 10), (10, 15), (20, 25), (40, 50)]}
\\
This means that if a source sentence has 4 tokens and the corresponding target sentence has 7 tokens, they are put in the first bucket and padded to length 5 for the encoder and length 10 for the decoder. A training sample having a source sentence with 21 tokens and a target sentence with 28 tokens does not fit into the bucket \((20, 25)\), so it is put into bucket \((40, 50)\) and padded accordingly. The English sentence ``Update Google Play Services'' and corresponding French sentence ``Mettre \`{a} jour les services Google Play'' are put into bucket \((5, 10)\) with vectors padded to the correct size: \texttt{[PAD Services Play Google Update]} and \texttt{[GO Mettre \`{a} jour les services Google Play EOS PAD]}. Reversing the source sentence was shown to improve results in \citep{smt_deep_learning}. The \texttt{GO} symbol always prefixes vectors passed into the decoder, the \texttt{EOS} symbol denotes the end of a sentence and the \texttt{PAD} symbol is used for padding. Behind the scenes, Tensorflow uses \(k\) different models for the \(k\) defined buckets. Without bucketing and padding, a model for every pair of lengths of source and target sentences would be created~\citep{website_tensorflow_tutorial}. 

The perplexity indicates how well the model of each bucket predicts the samples of the development set. A low perplexity means that the model is good at predicting the development sentences. Again, as with Moses, we used 80\% as training data and 20\% as development data. The (infinite) training loop was stopped after the perplexities did not improve anymore.
...
As in the previous chapters, we translate the same example sentence ``Open the settings to change your username'' to French.
The resulting translation produced by Tensorflow is the following: ``modifiez affichage Param\`{e}tres Param\`{e}tres votre utilisateur de votre nom''.
It should be noted that this translation is worse than the results of the previous two systems.
...
Additionally, the web application offers a functionality in terms of data analysis, called \emph{Term Variations}. Given a string, source and target language, it returns all different translations from Solr that were used among all apps to translate the string, together with a count. For example, for the English term ``settings'', the following translations are available in French: ``param\`{e}teres'', ``r\'{e}glages'', ``configuration'', ``settings''. %The last one indicates that at least one app did not translate ``settings'' correctly to French.

The source code of the web application is available on GitHub\footnote{Source code of web application: \url{https://github.com/wanze/AppTranslator}}.

%\rp{You cannot put links to internal networks, plus we might clean the machines at some point, it's better to just point to the source code on github if you didn't do it before.}
%\rp{The source code of the application is available online at ...}
...
The user interface to perform a translation consists of three steps. First of all, one needs to specify the source and target language, translation input and the system that produces the translations. The input can either be plain strings or an Android XML translation file (see Figure \ref{fig:web_app_index}). Second, there is the possibility to set the decoding options depending on the chosen translation system. The available options are described in the following section. Finally, the translation results are displayed in a table. If the decoder of the chosen translation system returns some statistics or debug information on how the translation results were generated, then this information is presented as well.

%\rp{you often use the word ``some'', which does not really tell anything to the reader, it's better to say something like ``the results are displayed together with additional debug informations, such as ...''}
...
To automatically evaluate the quality of machine translations, we used the standard \emph{BLEU} scoring algorithm~\cite{bleu_score}, which is commonly used for this task. This chapter first describes the BLEU scoring and explains the evaluation setup. Second, the BLEU scores that were achieved with the different translation systems are presented. Finally, the results of translating an existing app from English to French and German are presented.
...
The BLEU scores were computed in different evaluation runs for the results of translating English to French and German. \emph{K-fold cross-validation} was used to predict how the different models would perform if they were applied to independent data. The parallel corpus was split into 6 equal sized samples. One sample was always reserved as development set for Moses (Tuning) and Tensorflow (Evaluation), the remaining 5 samples were used to evaluate BLEU. For each evaluation run, four samples were merged into training data and a single sample was used as test set (see Figure~\ref{fig:evaluation_k_fold}). One sample contains 19'158 parallel sentences for English-French and 19'129 sentences for English-German.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.4 \textwidth]{images/evaluation_k_fold_cross_validation.png}
    \caption[Computing BLEU score with multiple runs using k-fold cross validation]{Splitting the parallel corpus for multiple evaluation runs: The white rectangle represents the test data, grey means training data and the black rectangle is the fixed development sample.}
    \label{fig:evaluation_k_fold}
\end{figure}

The Baseline System needed a special setup. First, the development set is ignored since there is no tuning or evaluation involved in this model. Second, the training data was indexed separately inside Solr for each evaluation run. This was achieved by creating new cores in Solr according to the following schema:
...
The parameter ``number of rows'' for the Baseline System (see Section \ref{cha:translation_process_baseline}) was set to 100 documents.


\subsection{Observations}

As we can observe, according to BLEU, Moses produces the best translations. As expected, if the language model component of Moses has less data available, the scores drop about 5 units. Still, the achieved BLEU scores are twice as much compared to Tensorflow and the Baseline System. While deep learning neural networks achieve a slightly higher BLEU score than phrase based systems in \cite{smt_deep_learning}, this does not apply to our context. One reason probably is that the corpus used in this work is too small. In machine translation literature, the translation models are usually trained on much larger amounts of data. The English-French parallel corpus from the \emph{European Parliament Proceedings}~\cite{website_parallel_corpus}, for instance, contains over 2 million sentences, which is \texttildelow 105 times more sentences than the one we used. Other than that, Moses has the advantage that its phrase table contains all possible words, while in Tensorflow, there is a fixed limit of the vocabulary size. Compared to Moses, Tensorflow also lacks a tuning process, where the optimal model parameters are determined.
...
Set location                                                            & Ort :                                                                                        & Standort w\"{a}hlen                                                              \\
...
The question if it is possible to provide high-quality translations for apps with machine learning can be answered with ``Yes''. Still, one might argue that this answer depends on the definition of \emph{high-quality}. On the one hand, good BLEU scores were achieved with a phrase-based system, as shown in Chapter \ref{cha:evaluation}. The BLEU scores for Moses are higher than the scores achieved in \cite{smt_phrase_based, smt_deep_learning} on the \emph{European Parliament parallel corpus} \cite{website_parallel_corpus}, though the context is different, so a direct comparison is not possible. The evaluation also showed that the produced translations with neural networks are worse compared to a pharse-based system. The results of Tensorflow could be improved by collecting more apps and thus having a larger corpora available for training. However, it still proofs our initial claim: Based on existing data and the similarity of sentences among different apps, we achieved a reasonable quality for machine translated sentences.  %that the concept of using existing data to translate new apps works. 
On the other hand, machine translated text is almost never perfect. Low quality app translations can do more harm than good, and users might get confused if they cannot understand the content. Because of that, machine translated text should be reviewed and corrected or completed by human translators.

\section{Future Work}


The current state of the project could be improved in many ways. Currently, all available translations are used to train the SMT models, which also includes wrong or poor translations. Existing translations could be rated based on metadata of the app they are coming from, e.g. number of downloads, average rating, number of reviews, etc. In general, the more existing high-quality translations that are collected to train the SMT models, the better the quality of the produced translations.

Another improvement would be to analyze what kind of sentences are translated well on which system. With this in mind, we could choose the optimal system based on the input data.

...
Decoding is the process of finding a translation for a given input sentence. Due to the big size of a phrase translation table, the search method is heuristic based. This means that the search algorithm can't explore the entire search space and therefore there is no guarantee that it will return the best translation.
...
\item \textbf{Pruning} Remove bad hypothesis early. Hypothesis that have translated the same number of input words are put into a stack. The number of hypotheses in a stack is limited, for example by keeping only \emph{k} hypothesis.
\end{itemize}

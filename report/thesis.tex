...
\item \textbf{Recombination} Hypothesis both having the same English words in the output can be reached by different paths. In the heuristic search they can be recombined and the worse hypothesis is dropped (see Figure~\ref{fig:related_work_hypothesis_recombination}).
...
\item Parse the resulting documents and extract the \texttt{app\_id} and \texttt{key} fields.
...
\end{enumerate}
Figure \ref{baseline_system_table_available_translations} shows some translations for the English word ``Settings''. The best translation according to the highest count among all apps is ``param\`{e}tres''.
...
However, if substring translation \rp{The simple algorithm is also able to translate substrings, please specify} is necessary, the following problems can occur:
...
\item Poor app translation quality directly impacts the translation results. For example, due to an error in the XML files, the English word ``the'' could be mapped to a sentence with many words. If the algorithm looks up ``the'', it considers the mapped sentence as valid translation.
\end{itemize} 
...
\rp{The listings are not particularly readable, maybe make an actual table?}
...
Here are some statistics on how the decoder derived the target sentence \rp{what do they tell us? some observations?}:
